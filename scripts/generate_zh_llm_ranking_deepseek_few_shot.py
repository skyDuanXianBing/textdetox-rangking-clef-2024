#!/usr/bin/env python3
"""
生成中文LLM评价排名文件（DeepSeek模型 Few-Shot）
基于各队伍的中文DeepSeek LLM Few-Shot评价结果生成汇总排名
"""
import pandas as pd
import numpy as np
import os
...
def generate_zh_llm_ranking_deepseek_few_shot():
    """生成中文DeepSeek LLM Few-Shot评价排名文件"""
    
    print("🚀 开始生成中文DeepSeek LLM Few-Shot评价排名")
    print("=" * 60)
    ...
    # 查找所有中文DeepSeek Few-Shot结果文件
    deepseek_pattern = os.path.join(llm_evolution_dir, "*_zh", "deepseek", "etd_few_shot_results_zh_ZHPrompt_deepseek.csv")
    deepseek_files = glob.glob(deepseek_pattern)
    
    print(f"✅ 找到 {len(deepseek_files)} 个中文DeepSeek LLM Few-Shot评价结果文件")
    ...
    for file_path in deepseek_files:
        try:
            ...
            ranking_data.append({
                'team_name': team_name,
                'evaluation_type': 'few_shot_deepseek',
                ...
                'file_path': f"llm_evolution/{team_name}_zh/deepseek/etd_few_shot_results_zh_ZHPrompt_deepseek.csv"
            })
    ...
    if not ranking_data:
        print("❌ 未找到有效的DeepSeek Few-Shot评价数据")
        return
    ...
    # 保存排名文件
    output_file = os.path.join(output_dir, "llm_evaluation_ranking_few_shot_deepseek.csv")
    ranking_df.to_csv(output_file, index=False)
    
    print(f"\n📊 DeepSeek Few-Shot排名结果 (按J-Score降序):")
    print("-" * 80)
    ...
    print(f"\n📁 DeepSeek Few-Shot排名文件已保存到: {output_file}")
    print(f"✅ 共处理 {len(ranking_df)} 个队伍的中文DeepSeek LLM Few-Shot评价结果")
    
    return ranking_df

if __name__ == "__main__":
    generate_zh_llm_ranking_deepseek_few_shot()
