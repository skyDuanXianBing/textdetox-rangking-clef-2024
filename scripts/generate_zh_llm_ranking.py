...
def generate_zh_llm_ranking():
    ...
    # 查找所有中文结果文件
    zh_pattern = os.path.join(llm_evolution_dir, "*_zh", "etd_zero_shot_results_zh_ZHPrompt.csv")
    zh_files = glob.glob(zh_pattern)
    
    print(f"✅ 找到 {len(zh_files)} 个中文LLM评价结果文件")
    ...
    for file_path in zh_files:
        try:
            ...
            ranking_data.append({
                ...
                'file_path': f"LLMEvolution/{team_name}_zh/etd_zero_shot_results_zh_ZHPrompt.csv"
            })
    ...
    # 保存排名文件
    output_file = os.path.join(output_dir, "llm_evaluation_ranking_zero_shot.csv")
    ...
    print(f"\n📁 排名文件已保存到: {output_file}")
    print(f"✅ 共处理 {len(ranking_df)} 个队伍的中文LLM评价结果")

if __name__ == "__main__":
    generate_zh_llm_ranking()
