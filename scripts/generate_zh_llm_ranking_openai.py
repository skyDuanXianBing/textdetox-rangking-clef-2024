#!/usr/bin/env python3
"""
生成中文LLM评价排名文件（OpenAI模型）
基于各队伍的中文OpenAI LLM评价结果生成汇总排名
"""
import pandas as pd
import numpy as np
import os
...
def generate_zh_llm_ranking_openai():
    """生成中文OpenAI LLM评价排名文件（Few-Shot）"""

    print("🚀 开始生成中文OpenAI LLM评价排名（Few-Shot）")
    print("=" * 60)
    ...
    # 查找所有中文OpenAI结果文件（Few-Shot）
    openai_pattern = os.path.join(llm_evolution_dir, "*_zh", "openai", "etd_few_shot_results_zh_ZHPrompt_openai.csv")
    openai_files = glob.glob(openai_pattern)
    
    print(f"✅ 找到 {len(openai_files)} 个中文OpenAI LLM评价结果文件")
    ...
    for file_path in openai_files:
        try:
            ...
            ranking_data.append({
                'team_name': team_name,
                'evaluation_type': 'few_shot_openai',
                ...
                'file_path': f"llm_evolution/{team_name}_zh/openai/etd_few_shot_results_zh_ZHPrompt_openai.csv"
            })
    ...
    # 保存排名文件
    output_file = os.path.join(output_dir, "llm_evaluation_ranking_few_shot_openai.csv")
    ranking_df.to_csv(output_file, index=False)
    
    print(f"\n📊 OpenAI排名结果 (按J-Score降序):")
    print("-" * 80)
    ...
    print(f"\n📁 OpenAI排名文件已保存到: {output_file}")
    print(f"✅ 共处理 {len(ranking_df)} 个队伍的中文OpenAI LLM评价结果")
    
    return ranking_df

if __name__ == "__main__":
    generate_zh_llm_ranking_openai()
